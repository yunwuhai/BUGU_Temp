...
type Overload struct {
	*gorm.Model
	OverloadIntroduction string `json:"overload_introduction" gorm:"type:varchar(255)"`
	OverloadMaskCode     string `json:"overload_mask_code" gorm:"type:varchar(255)"`
	ModuleID             uint `json:"module_id" gorm:"type:varchar(64)"`
}
...
func (o Overload) Count(db *gorm.DB) (int, error) {
	...
	if err := db.Model(&o).Where("deleted_at is null").Count(&count).Error; err != nil {
		return 0, err
	}
	return count, nil
}