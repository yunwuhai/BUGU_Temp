...
func (d *Dao) CreateModule(moduleName, moduleIntroduction, pluginName string, IsFunc uint) error {
	pa := model.Plugin{
...
func (d *Dao) UpdateModule(id uint, moduleName, moduleIntroduction, pluginName string, IsFunc uint) error {
	m := model.Module{
		ModuleName: moduleName,
	}
	module, err := m.GetByModuleName(d.engine)
